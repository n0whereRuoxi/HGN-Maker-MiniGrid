The files in this directory consist of those that should be necessary to reproduce the Satellite domain results published in "Learning Methods to Generate Good Plans: Integrating HTN Learning and Reinforcement Learning" by Chad Hogg, Ugur Kuter, and Hector Munoz-Avila published in Proceedings of the Twenty-Fourth AAAI Conference on Artificial Intelligence (AAAI-10).
...
`raw_probs/*.raw_prob` - A set of 800 classical planning problems in the Satellite domain, formatted for use by the FastForward planner.
...
`qmaker_output/*_domain.htn` - An HTN representation of the Satellite domain with methods learned from the associated problem in `raw_probs/` and all problems that precede it in `qmaker_problem_order.txt`.  The algorithm for producing each of these is as follows:
...
`phase2_htn_probs/*.htn_prob` - A set of 800 HTN planning problems in the Satellite domain, formatted for use by HTN-Maker and related programs.  Any overlap between these and the problems in `raw_probs/` is strictly coincidental.
...
`phase3_raw_probs/*.strips_prob` - A set of 220 classical planning problems in the Satellite domain, formatted for use by FastForward and SGPlan6.  Any overlap between these problems and those in `raw_probs/` and `phase2_htn_probs/` is strictly coincidental.  There are 20 problems each of size 2 to 12.
